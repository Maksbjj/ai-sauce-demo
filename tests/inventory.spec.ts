...
test.describe('Inventory Page - Display', () => {
    test('Should display the Products title on the inventory page', async ({ loggedIn, inventoryPage }) => {
        await test.step('Verify page title is visible', async () => {
            await softExpect(inventoryPage.pageTitle).toBeVisible()
        })
    })

    test('Should display 6 products on the inventory page', async ({ loggedIn, inventoryPage }) => {
        await test.step('Verify 6 product items are displayed', async () => {
            await softExpect(inventoryPage.productItems).toHaveCount(6)
        })
    })

    test('Should display name, price, and Add to cart button for each product', async ({ loggedIn, inventoryPage }) => {
    ...
    test('Should display sort dropdown with default option Name (A to Z)', async ({ loggedIn, inventoryPage }) => {
    ...
    test('Should logout and redirect to login page via burger menu', async ({ loggedIn, page, inventoryPage }) => {
