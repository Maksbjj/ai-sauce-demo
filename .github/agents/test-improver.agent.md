...
Before analysing any test file, read these files to understand fixtures and what state is already established before tests run:

- `src/fixtures/fixtures.ts` — per-test fixture setup and page object injection
- `src/pages/` — page objects and their methods
- `src/utils/assertions.ts` — soft assertion helpers
...
For any page object method whose behaviour is unclear from the test alone, read the relevant parts of the page object to understand whether it makes additional calls or has retry logic.
